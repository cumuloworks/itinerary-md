...
export function makeDefaultServices(
  ...
  const legacyCurrency = (policy as { currencyFallback?: string | null })
    .currencyFallback;
  if (legacyTz != null) {
    try {
      console.warn(
        '[itmd/core] Policy.tzFallback is deprecated and will be removed in 0.2.0. Use `defaultTimezone`.'
      );
    } catch {}
  }
  if (legacyCurrency != null) {
    try {
      console.warn(
        '[itmd/core] Policy.currencyFallback is deprecated and will be removed in 0.2.0. Use `defaultCurrency`.'
      );
    } catch {}
  }
