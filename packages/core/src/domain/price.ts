...
export function normalizePriceLine(
  rawLine: string,
  ...
  const PATTERNS = [
    ...
    new RegExp(
      `^(?<sym>A\\$|C\\$|HK\\$|S\\$|[€¥£₩₫฿₱₽₺$])\\s*(?<num>${NUM_RE.source})(?=\\s|$)`
    ),
  ];
  ...
  if (!match || !kind) {
    ...
    if (!currency && numM) {
      ...
      if (defaultCurrency?.trim()) {
        ...
        if (!warnings.includes('currency-not-detected'))
          warnings.push('currency-not-detected');
      ...
      const tokens: PriceTokens = [
        { kind: 'number', raw: rawNumStr, normalized: numNorm },
      ];
      return {
        type: 'itmdPrice',
        rawLine,
  ...
  const currency =
    code || (sym ? SYMBOL_TO_CODE[sym] : undefined) || defaultCurrency;
  ...
  const source: 'inline' | 'defaultCurrency' | 'symbolInferred' | undefined =
    code
      ? 'inline'
      : sym
        ? 'symbolInferred'
