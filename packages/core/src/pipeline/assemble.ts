  ...
  for (let i = 0; i < children.length; i += 1) {
    ...
    if (
      ...
      const d = ((): {
        ...
        const m = text.match(
          /^(\d{4}-\d{2}-\d{2})(?:\s*@([A-Za-z0-9_./+:-]+))?/
        );
        if (!m) return null;
        const date = m[1];
        const tzRaw = m[2];
        const tzCheck = tzRaw
          ? sv.tz.coerce(tzRaw, sv.policy.defaultTimezone ?? undefined)
          : { tz: undefined, valid: true };
      ...
      if (d?.date) {
        const headingNode: ITMDHeadingNode = {
          ...
          data: {
            itmdDate: { dateISO: d.date, timezone: d.timezone },
            hProperties: { 'data-itmd-date': d.date },
          },
        } as ITMDHeadingNode;
    ...
    const blockChildren = Array.isArray(bq.children)
      ? (bq.children as unknown as Parent['children'])
      : ([] as Parent['children']);
    const firstPara = blockChildren.find(
      (n) => (n as unknown as { type?: string }).type === 'paragraph'
    ) as (Parent & { type: 'paragraph' }) | undefined;
    ...
    for (const nodeAbs of blockChildren) {
      ...
      if (t === 'paragraph') {
        const inlinePara = ((
          nodeAbs as unknown as Parent & { type: 'paragraph' }
        ).children ?? []) as unknown as PhrasingContent[];
      ...
      if (t === 'list') {
        ...
        const flush = () => {
          if (!group) return;
          if (group.kind === 'meta' && group.entries.length > 0)
            bodySegs.push({ kind: 'meta', entries: group.entries });
        ...
        const listOrdered = !!(list as unknown as { ordered?: boolean })
          .ordered;
        ...
        for (const it of items) {
          ...
          const para0 = kids.find(
            (n) => (n as unknown as { type?: string })?.type === 'paragraph'
          ) as (Parent & { type: 'paragraph' }) | undefined;
          const inlineLi = (para0?.children ??
            []) as unknown as PhrasingContent[];
          const raw = para0
            ? mdastToString(para0 as unknown as any)
            : mdastToString(li as unknown as any);
          ...
          if (isKv) {
            ...
            const valueNodes =
              inlineLi.length > 0
                ? sliceInlineNodes(inlineLi, valStart, rawText.length)
                : ([
                    { type: 'text', value: rawText.slice(valStart).trim() },
                  ] as any);
            if (key) {
            ...
            const inlineNodes =
              inlineLi.length > 0
                ? inlineLi
                : ([{ type: 'text', value: textTrimmed }] as any);
    ...
    for (const seg of bodySegs) {
      ...
      for (const e of m.entries) {
        if (e.key === 'price' || e.key === 'cost') {
          ...
          const priceNode = normalizePriceLine(
            rawStr,
            sv.policy.defaultCurrency ?? undefined
          );
    ...
    if (priceEntries.length > 0) {
      const prevData = ((built as unknown as { data?: Record<string, unknown> })
        .data || {}) as Record<string, unknown>;
    ...
    if (currentDateISO) {
      const prev = ((built as unknown as { data?: Record<string, unknown> })
        .data || {}) as Record<string, unknown>;
