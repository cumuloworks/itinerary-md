...
describe('lexLine', () => {
  ...
  it('detects ::, at, routeDash only outside parentheses/links/code', () => {
    const line =
      '[08:00] lunch at Cafe :: X - Y [link: http://a-b.com] (`code - span`)';
    const t = lexLine(line, {}, sv);
    const kinds = new Set(t.seps.map((s) => s.kind));
    expect(kinds.has('doublecolon')).toBe(true);
    expect(kinds.has('at')).toBe(true);
    expect(kinds.has('routeDash')).toBe(true);
  });
  ...
  it('detects via (outside only)', () => {
    ...
    const kinds = new Set(t.seps.map((s) => s.kind));
    expect(kinds.has('from')).toBe(true);
    expect(kinds.has('via')).toBe(true);
    expect(kinds.has('to')).toBe(true);
  });
  ...
  it('ignores from/to inside parentheses', () => {
    ...
    const t = lexLine(line, {}, sv);
    const kinds = new Set(t.seps.map((s) => s.kind));
    expect(kinds.has('from')).toBe(false);
    expect(kinds.has('to')).toBe(false);
  });

  it('ignores from/to inside link text', () => {
    const line =
      '[08:00] activity go [from here to there](https://example.com)';
    const t = lexLine(line, {}, sv);
    const kinds = new Set(t.seps.map((s) => s.kind));
    expect(kinds.has('from')).toBe(false);
    expect(kinds.has('to')).toBe(false);
  ...
  it('ignores via inside parentheses/link/code', () => {
    ...
    const t2 = lexLine(
      '[08:00] activity go [via somewhere](https://ex) to there',
      {},
      sv
    );
