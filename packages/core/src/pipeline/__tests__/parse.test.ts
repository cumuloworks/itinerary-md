...
describe('parseHeader', () => {
  ...
  it('destination: single at via :: (title is null when omitted)', () => {
    ...
    const d = header.destination as Extract<
      typeof header.destination,
      { kind: 'single' }
    >;
    const at = d.at;
    expect(
      mdastToString({ type: 'paragraph', children: at } as unknown as Parent)
    ).toContain('Cafe Luna');
  });
  ...
  it('link preservation: keep link in single(::) at', () => {
    ...
    expect(at.some((n) => isLink(n) && n.url === 'https://cafe.example')).toBe(
      true
    );
  });
  ...
  it('link preservation: keep links in dashPair from/to', () => {
    ...
    const isLink = (n: PhrasingContent): n is import('mdast').Link =>
      (n as unknown as { type?: string }).type === 'link';
    expect(
      (d.from as PhrasingContent[]).some(
        (n) => isLink(n) && n.url === 'https://a.example'
      )
    ).toBe(true);
    expect(
      (d.to as PhrasingContent[]).some(
        (n) => isLink(n) && n.url === 'https://b.example'
      )
    ).toBe(true);
  });
  ...
  it('link preservation: keep links in fromTo from/to', () => {
    ...
    expect(
      (d.from as PhrasingContent[]).some(
        (n) => isLink(n) && n.url === 'https://tokyo.example'
      )
    ).toBe(true);
    expect(
      (d.to as PhrasingContent[]).some(
        (n) => isLink(n) && n.url === 'https://london.example'
      )
    ).toBe(true);
  });
  ...
  it('destination: from ... to ... yields fromTo (link preserved)', () => {
    const line =
      '[am@Asia/Tokyo] - [18:45@Europe/London] flight JL043 [Official site](https://www.jal.co.jp/) from [Tokyo Haneda (HND)](https://haneda-airport.jp/) to [London Heathrow (LHR)](https://www.heathrow.com/)';
    const tokens = lexLine(line, {}, sv);
    const header = parseHeader(tokens, [], sv);
    expect(header.destination?.kind).toBe('fromTo');
    const d = header.destination as Extract<
  ...
  it('destination: from A via B via C to D → normalized to from=A, to=D', () => {
    const line =
      '[08:00] flight JL from Tokyo via Dubai via Frankfurt to London';
    const tokens = lexLine(line, {}, sv);
    const header = parseHeader(tokens, [], sv);
    expect(header.destination?.kind).toBe('fromTo');
    const d = header.destination as Extract<
  ...
  it('destination: :: A - B - C → from=A, via=[B], to=C', () => {
    ...
    const d = h.destination as Extract<
    ...
    expect(
      mdastToString({ type: 'paragraph', children: d.to } as unknown as Parent)
    ).toBe('C');
  ...
  it('retains offsets of time/destination/title', () => {
    ...
    if (!dpos || !dpos.from || !dpos.to)
      throw new Error('missing destination positions');
