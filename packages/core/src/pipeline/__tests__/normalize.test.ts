...
describe('normalize: time normalization and ISO/TZ', () => {
  ...
  it('generates startISO for dateISO + hh:mm (considering fallback TZ)', () => {
    ...
    if (!out.time || out.time.kind !== 'point')
      throw new Error('time should be point');
    expect(out.time.startISO?.startsWith('2025-03-15T08:30')).toBe(true);
    expect(out.time.startISO).toMatch(
  ...
  it('does not generate ISO when marker is present', () => {
    const sv = makeDefaultServices({ defaultTimezone: 'Europe/Madrid' });
    const out = normalizeHeader(
      { eventType: 'breakfast', time: { kind: 'marker', marker: 'am' } },
      { baseTz: undefined, dateISO: '2025-03-15' },
      sv
    );
    if (!out.time || out.time.kind !== 'marker')
      throw new Error('time should be marker');
  ...
  it('range: generates next-day ISO considering end.dayOffset(+1)', () => {
    ...
    if (!out.time || out.time.kind !== 'range')
      throw new Error('time should be range');
