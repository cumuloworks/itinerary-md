...
describe('assemble', () => {
  ...
  it('splits native^local in title and destination(single)', () => {
          ...
          children: [
            {
              ...
              children: [
                {
                  type: 'text',
                  value: '[08:00] lunch タイトル^Title :: 店^Shop',
                },
              ],
            },
          ],
    ...
    const titleAltText = mdastToString({
      type: 'paragraph',
      children: evt.title_alt || [],
    } as unknown as Parent);
    expect(titleText).toBe('タイトル');
    expect(titleAltText).toBe('Title');
    // at split
    const atText = mdastToString({
      type: 'paragraph',
      children: evt.destination.at || [],
    } as unknown as Parent);
    const atAltText = mdastToString({
      type: 'paragraph',
      children: evt.destination.at_alt || [],
    } as unknown as Parent);
  ...
  it('splits native^local in destination(dashPair/fromTo)', () => {
                ...
                children: [
                  {
                    ...
                    value:
                      kind === 'dashPair'
                        ? '[08:00] flight :: 出発^From - 到着^To'
                        : '[08:00] flight from 出発^From to 到着^To',
                  },
                ],
              },
            ],
    ...
    for (const kind of ['dashPair', 'fromTo'] as const) {
      ...
      const fromText = mdastToString({
        type: 'paragraph',
        children: evt.destination.from || [],
      } as unknown as Parent);
      const fromAltText = mdastToString({
        type: 'paragraph',
        children: evt.destination.from_alt || [],
      } as unknown as Parent);
      const toText = mdastToString({
        type: 'paragraph',
        children: evt.destination.to || [],
      } as unknown as Parent);
      const toAltText = mdastToString({
        type: 'paragraph',
        children: evt.destination.to_alt || [],
      } as unknown as Parent);
                  ...
                  children: [
                    {
                      type: 'paragraph',
                      children: [
                        { type: 'text', value: '- note: use audio guide' },
                      ],
                    },
                  ],
                },
              ...
              children: [
                {
                  type: 'text',
                  value:
                    '[06:30] breakfast Traditional Japanese breakfast :: hotel',
                },
                { type: 'text', value: '\n' },
  ...
  it('extracts key: value from lists inside blockquote into body.meta (order preserved)', () => {
    ...
    const entries = metas.flatMap(
      (m: any) => m.entries as Array<{ key: string; value: PhrasingContent[] }>
    );
    const get = (k: string) => entries.find((e) => e.key === k)?.value ?? [];
    expect(
      mdastToString({
  ...
  it('meta: ignore leading "- key: value" hyphen/space when keying; preserve links in value', () => {
    ...
    const priceInline = (entries.find((e) => e.key === 'price')?.value ??
      []) as PhrasingContent[];
    const isLink = (n: PhrasingContent): n is import('mdast').Link =>
      (n as unknown as { type?: string }).type === 'link';
    expect(
      priceInline.some((n) => isLink(n) && n.url === 'https://pay.example')
    ).toBe(true);
  });
  ...
  it('meta: extract key:value from entire listItem text even without paragraph', () => {
    ...
    const noteV = (entries.find((e) => e.key === 'note')?.value ??
      []) as PhrasingContent[];
    expect(
      mdastToString({
        type: 'paragraph',
                  ...
                  children: [
                    {
                      type: 'paragraph',
                      children: [
                        { type: 'text', value: '- note: value: with: colons' },
                      ],
                    },
                  ],
                },
          ...
          children: [
            {
              type: 'paragraph',
              children: [
                { type: 'text', value: '[] flight AA000 from Tokyo to London' },
              ],
            },
          ],
        },
