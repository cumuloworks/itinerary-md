...
export function parseTimeSpan(
  line: string
): { time: TimeSpan; consumed: number } | { time: null; consumed: 0 } {
  const m = line.match(/^\[(.*?)\](?:\s*-\s*\[(.*?)\])?\s*/);
  if (!m) return { time: null, consumed: 0 } as const;
  const startRaw = (m[1] ?? '').trim();
  const endRaw = (m[2] ?? '').trim();
  if (startRaw === '')
    return { time: { kind: 'none' }, consumed: m[0].length } as const;
  const ls = startRaw.toLowerCase();
  const marker = ls === 'am' || ls === 'pm' ? (ls as 'am' | 'pm') : null;
  if (marker)
    return { time: { kind: 'marker', marker }, consumed: m[0].length } as const;
...
function normalizeSoftBreaksInTextNodes(
  nodes: PhrasingContent[]
): PhrasingContent[] {
  const isWordChar = (ch: string | undefined): boolean =>
    !!ch && /[A-Za-z0-9]/.test(ch);
  const getFirstTextChar = (
    arr: PhrasingContent[],
    fromIdx: number
  ): string | undefined => {
...
function splitHeadAndDestUsingSeps(
  ...
  const seps = (tokens.seps || []).filter(
    (s) => s.kind === 'doublecolon' || s.kind === 'at' || s.kind === 'from'
  ) as Array<{ kind: SepKind; index: number }>;

  let chosen: { kind: SepKind; rawIdx: number } | null = null;
  const sorted = seps
    .filter((s) => s.index >= consumed)
...
function parseHead(headRaw: string): {
  ...
  const title = titleText
    ? ([
        { type: 'text', value: titleText } as unknown as PhrasingContent,
      ] as PhrasingContent[])
    : null;
...
export function parseHeader(
  tokens: LexTokens,
  mdInline: PhrasingContent[],
  _sv: Services
): ParsedHeader {
  const line = tokens.raw.trim();
  const ts = parseTimeSpan(line);
  const { headRaw, destRaw, sep } = splitHeadAndDestUsingSeps(
    line,
    tokens,
    ts.consumed
  );
  ...
  const inlineFullText = hasInline
    ? mdastToString(toParagraph(mdInline))
    : tokens.raw.trim();
  const sliceInline = (start: number, end: number): PhrasingContent[] => {
    ...
    return substr
      ? ([{ type: 'text', value: substr }] as unknown as PhrasingContent[])
      : [];
  };
  const pickInline = (start: number, end: number): PhrasingContent[] =>
    normalizeSoftBreaksInTextNodes(
      sliceInline(start, end).filter(
        (n) => mdastToString(toParagraph([n])).trim() !== ''
      )
    );
  ...
  if (headTitleStartInline < headEndInline) {
    ...
    if (mdastToString(toParagraph(titleInline)).trim() === '')
      titleInline = null;
  }
  ...
  if (destRaw) {
    ...
    const destEndInline =
      destStartInline >= 0
        ? destStartInline + destRaw.length
        : inlineFullText.length;
    ...
    if (!destination && sep === 'from') {
      ...
      if (toIdxRel >= 0) {
        ...
        while (true) {
          ...
          segsAbs.push({
            s: destStartInline + cursorRel,
            e: destStartInline + nextVia,
          });
        ...
        const vias: PhrasingContent[][] =
          segsAbs.length > 1
            ? segsAbs.slice(1).map(({ s, e }) => pickInline(s, e))
            : [];
        destination = {
          kind: 'fromTo',
    ...
    if (!destination && !handledFromTo) {
      ...
      if (fromIdxRel >= 0) {
        const toIdxRel = lowerDest.indexOf(
          ' to ',
          fromIdxRel + ' from '.length
        );
        if (toIdxRel >= 0) {
    ...
    if (!destination && idx >= 0) {
      ...
      if (partsAbs.length >= 2) {
        ...
        const vias: PhrasingContent[][] = partsAbs
          .slice(1, -1)
          .map(({ s, e }) => normalizeSoftBreaksInTextNodes(sliceInline(s, e)));
        destination = {
          kind: 'dashPair',
      ...
      destination = {
        kind: 'single',
        at: normalizeSoftBreaksInTextNodes(
          sliceInline(destStartInline, destEndInline)
        ),
      } as ParsedHeader['destination'];
      positions.destination = {
  ...
  const title = ((): PhrasingContent[] | null => {
    ...
    if (head.title && mdastToString(toParagraph(head.title)).trim() !== '')
      return head.title;
    return null;
  })();
  if (titleInline)
    positions.title = { start: headTitleStartInline, end: headEndInline };
  return {
