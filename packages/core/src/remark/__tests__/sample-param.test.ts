...
function toItineraryEvents(root: Root): { type: string; titleText?: string }[] {
  ...
  visit(root as unknown as { type: string }, 'itmdEvent', (n: unknown) => {
    ...
    const titleText = Array.isArray(node.title)
      ? (node.title as PhrasingContent[])
          .map((t) =>
            typeof (t as unknown as { value?: unknown }).value === 'string'
              ? (t as unknown as { value: string }).value
              : ''
          )
          .join('')
      : undefined;
      ...
      for (const line of eventLines) {
        it(`parses line: ${line}`, () => {
          ...
          const ran = unified()
            ...
            .use(remarkItinerary, {})
            .runSync(tree);
          const events = toItineraryEvents(
            ran as unknown as import('mdast').Root
          );
