...
function sliceNode(
  ...
  if (node.type === 'text') {
    ...
    if (newChildren.length > 0) {
      ...
      (cloned as Parent & { children: PhrasingContent[] }).children =
        newChildren;
...
export function splitRichInlineByCaret(nodes: RichInline): {
  left: RichInline;
  right: RichInline | null;
} {
  try {
    const plain =
      mdastToString({
