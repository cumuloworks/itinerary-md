...
export const remarkItineraryAlert: Plugin<[], Root> = () => {
  return (tree: Root) => {
    visit(tree, 'blockquote', (node: Blockquote, index, parent) => {
      ...
      const m = /^\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*/i.exec(
        p0.value
      );
      if (!m) return;
      ...
      if (remainingText.trim())
        inlineTitle.push({ type: 'text', value: remainingText });
      if (paragraph.children.length > 1)
        inlineTitle.push(...(paragraph.children.slice(1) as PhrasingContent[]));
      ...
      if (node.children.length > 1)
        alertNode.children.push(...node.children.slice(1));
