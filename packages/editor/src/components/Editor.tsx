		...
		<div className="h-full flex flex-col min-h-0 gap-4">
			...
			<div className={containerClass}>
				{(topbar.viewMode === "split" || topbar.viewMode === "editor") && (
					<div
						className={`${topbar.viewMode === "split" ? "md:basis-1/2 basis-1/3" : "flex-1"} min-w-0 min-h-0`}
					>
						<div className="px-2 py-1 bg-gray-100 border-b border-gray-300 font-medium text-sm text-gray-600">
							Editor
						</div>
				...
				{(topbar.viewMode === "split" || topbar.viewMode === "preview") && (
					<div
						className={`${topbar.viewMode === "split" ? "md:basis-1/2 basis-2/3" : "flex-1"} min-w-0 min-h-0`}
					>
						<div className="px-2 py-1 flex justify-between bg-gray-100 border-b border-gray-300 font-medium text-sm text-gray-600 group">
							...
							<button
								type="button"
								className="text-sm text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity"
								onClick={() => updateTopbar({ showMdast: !topbar.showMdast })}
							>
								<Bug size={12} />
							</button>
						</div>
