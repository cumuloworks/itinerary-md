...
const extractHeadingDates = (
  ...
  for (const n of nodes) {
    ...
    if (
      n.type === 'itmdHeading' &&
      typeof (n as { dateISO?: string }).dateISO === 'string'
    ) {
...
export const Statistics: React.FC<StatisticsProps> = ({
  ...
  const summary = React.useMemo(
    () => extractHeadingDates(root?.children),
    [root]
  );

  const { total, totalFormatted, breakdownFormatted, toCurrency } =
    React.useMemo(() => {
      ...
      const toCurrency = normalizeCurrencyCode(
        currency ??
          (typeof frontmatter?.currency === 'string'
            ? (frontmatter?.currency as string)
            : undefined),
        'USD'
      );
      ...
      for (const node of children) {
        ...
        const prices = Array.isArray(ev.data?.itmdPrice)
          ? (ev.data?.itmdPrice as PriceEntryLike[])
          : [];
        ...
        for (const p of prices) {
          ...
          for (const tok of toks) {
            ...
            const from = normalizeCurrencyCode(
              tok.normalized?.currency || tok.currency || to,
              to
            );
            const amt = Number(
              String(tok.normalized?.amount || tok.amount || '')
            );
            ...
            if (converted == null) {
              if (from === to) {
                ...
                const hasFrom =
                  typeof (ratesData.rates as Record<string, number>)[from] ===
                  'number';
                const hasTo =
                  typeof (ratesData.rates as Record<string, number>)[to] ===
                  'number';
                if (hasFrom && hasTo) {
                  converted = convertAmountUSDBase(
                    amt,
                    from,
                    to,
                    ratesData.rates as Record<string, number>
                  );
                } else {
  ...
  const budgetDisplay = React.useMemo(() => {
    ...
    if (typeof raw === 'number')
      parsed = parseAmountWithCurrency(String(raw), undefined);
    else if (typeof raw === 'string')
      parsed = parseAmountWithCurrency(raw, undefined);
    ...
    if (converted == null) {
      ...
      else if (ratesData?.rates?.[from] && ratesData?.rates?.[toCurrency])
        converted =
          convertAmountUSDBase(amt, from, toCurrency, ratesData.rates) ?? null;
      else converted = null;
    }
    ...
    const remainingLabel =
      remaining >= 0
        ...
        : `${remainingFormatted} over`;
    const usedPct =
      converted > 0
        ? Math.min(
            999,
            Math.max(0, Math.round(((total || 0) / converted) * 100))
          )
        : 0;
  ...
  return (
    <div className="flex flex-wrap justify-evenly rounded border border-gray-300 bg-gray-50 py-4">
      <div className="flex basis-1/2 flex-col items-center justify-center rounded-lg p-4">
        <div
          className="flex flex-wrap items-baseline font-bold break-all whitespace-pre"
          aria-live="polite"
        >
          <p
            className={`text-3xl ${budgetDisplay?.remaining !== undefined && budgetDisplay?.remaining !== null ? (budgetDisplay.remaining >= 0 ? 'text-emerald-600' : 'text-red-600') : 'text-gray-800'}`}
          >
            {totalFormatted ?? '—'}
          </p>
          {budgetDisplay && (
        ...
        <div className="mt-2 w-full px-4">
          <div className="mx-auto flex max-w-60 justify-center gap-x-10 text-center">
            <div className="flex flex-1 flex-col items-center">
              <div className="flex items-center gap-1">
                <Plane size={20} className="text-gray-600" />
              </div>
              <div className="text-sm font-semibold text-gray-800">
                {breakdownFormatted?.transportation ?? '—'}
              </div>
            </div>
            <div className="flex flex-1 flex-col items-center">
              <FerrisWheel size={20} className="text-gray-600" />
              <div className="text-sm font-semibold text-gray-800">
                {breakdownFormatted?.activity ?? '—'}
              </div>
            </div>
            <div className="flex flex-1 flex-col items-center">
              <BedDouble size={20} className="text-gray-600" />
              <div className="text-sm font-semibold text-gray-800">
                {breakdownFormatted?.stay ?? '—'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex basis-1/2 flex-col items-center justify-center rounded-lg p-4">
        <div className="flex flex-col items-center justify-center gap-1 text-2xl font-bold whitespace-nowrap text-gray-800">
