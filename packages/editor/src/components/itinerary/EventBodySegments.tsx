...
const MetaList: React.FC<{ entries: MetaEntry[]; className?: string }> = ({
  entries,
}) => {
...
export const EventBodySegments: React.FC<{
  ...
  return (
    <div className={`mt-0.5 border-t pt-2 ${borderClass}`}>
      {bodySegments.map((seg) => {
        ...
        if ((seg as { kind?: string }).kind === 'meta') {
          ...
          const priceEntries = (m.entries || []).filter(
            (e) => e != null && (e.key === 'price' || e.key === 'cost')
          );
          const restEntries = (m.entries || []).filter(
            (e) => e != null && e.key !== 'price' && e.key !== 'cost'
          );
        ...
        if ((seg as { kind?: string }).kind === 'list') {
          ...
          if (isOrdered) {
            return (
              <ol
                key={`list-${start ?? 'ol'}`}
                className="ml-6 list-decimal marker:text-base marker:font-bold marker:text-blue-600"
                start={start}
              >
                {l.items.map((it) => {
                  const keyStr = it
          ...
          return (
            <ul
              key={`list-ul-${l.items.length}-${(l.items[0] || [])
                ...
                .join('-')
                .slice(0, 16)}`}
              className="ml-6 list-disc marker:text-base marker:font-bold marker:text-blue-600"
            >
              {l.items.map((it) => {
                const keyStr = it
                  .map((seg) => `${seg.text}-${seg.url ?? ''}`)
