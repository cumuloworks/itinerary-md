...
export function EventActionsMenu({
  ...
  const startDt = startISO
    ? DateTime.fromISO(startISO, { zone: timezone || undefined })
    : null;
  const endDt = endISO
    ? DateTime.fromISO(endISO, { zone: timezone || undefined })
    : null;

  const triggerBaseClass =
    'flex items-center justify-center rounded-full size-7 hover:brightness-110 hover:scale-110 transition';
  const menuItemClass =
    'flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-sm text-gray-800 outline-none hover:bg-gray-100';
  const contentClass =
    'min-w-44 rounded-md bg-white p-1 shadow-lg border border-gray-200 focus:outline-none z-50';
  ...
  const googleCalendarUrl = buildGoogleCalendarUrl({
    ...
    location: destinationText || undefined,
  });
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <button
            ...
            className={`${triggerBaseClass} ${iconBgClass}`}
          >
