...
const MarkdownPreviewComponent: FC<MarkdownPreviewProps> = ({
	...
	const { reactContent, root, parsedFrontmatter, isItmd } =
		React.useMemo(() => {
			try {
				...
				const normalizedTimezone = isValidIanaTimeZone(fmTimezoneRaw || "")
					? (fmTimezoneRaw as string)
					: undefined;
				const normalizedCurrency = normalizeCurrencyCode(
					fmCurrencyRaw || currency,
					currency || "USD",
				);
				...
				const prefixLength =
					typeof fm.content === "string"
						? content.length - fm.content.length
						: 0;
				...
				const getNodeDateAttr = (n: unknown): string | undefined => {
					try {
						const d = (
							n as { data?: { hProperties?: Record<string, unknown> } }
	...
	const frontmatterTz =
		typeof safeParsedFrontmatter?.timezone === "string"
			? (safeParsedFrontmatter.timezone as string)
			: undefined;
	React.useEffect(() => {
		if (frontmatterTz && !isValidIanaTimeZone(frontmatterTz)) {
			notifyError(
				`Frontmatter timezone "${frontmatterTz}" is invalid. Using fallback.`,
			);
		}
	}, [frontmatterTz]);
	...
	React.useEffect(() => {
		...
		let nodes = Array.from(
			container.querySelectorAll<HTMLElement>(
				"[data-itin-line-start], [data-itin-line-end]",
			),
		);
		if (nodes.length === 0) {
			nodes = Array.from(
		...
		const margin = 8;
		const visible =
			tRect.bottom > cRect.top + margin && tRect.top < cRect.bottom - margin;
		if (visible) return;
		const delta =
	...
	return (
		<div
			ref={containerRef}
			className="markdown-preview h-full px-8 py-4 bg-white overflow-auto space-y-4"
		>
			{title && (
				<h1 className="text-4xl font-bold text-gray-900 mt-6 ml-0 tracking-tight">
					{title}
				</h1>
			)}
			{description && (
			...
			{isItmd && (
				<Statistics
					root={root as any}
					frontmatter={safeParsedFrontmatter}
					currency={currency}
					rate={rate}
				/>
			)}
