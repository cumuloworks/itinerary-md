...
describe("Statistics", () => {
	...
	it("aggregates total and breakdown (transportation/activity/stay) in USD", () => {
		...
		expect(
			(document.body.textContent || "").match(/0\.00/g)?.length || 0,
		).toBeGreaterThan(0);
	});
	...
	it("shows budget from frontmatter and the difference vs total (1:1)", () => {
		...
		render(
			<Statistics
				root={root}
				frontmatter={{ currency: "USD", budget: "250 USD" }}
			/>,
		);
	...
	it("prioritizes manual rate props to convert budget/total", () => {
		...
		render(
			<Statistics
				root={root}
				frontmatter={{ currency: "USD", budget: "200 EUR" }}
				rate={{ from: "EUR", to: "USD", value: 2 }}
			/>,
		);
