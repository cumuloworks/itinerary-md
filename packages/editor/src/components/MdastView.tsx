...
function sanitizeNode(node: any, allowAllPlainObjects = false): any {
	...
	if (Array.isArray(node))
		return node.map((v) => sanitizeNode(v, allowAllPlainObjects));
...
const MdastViewComponent: FC<MdastViewProps> = ({
	content,
	timezone,
	currency,
}) => {
	...
	return (
		<div className="h-full w-full bg-white p-2">
			{tree ? (
				...
				<pre className="h-full w-full overflow-auto text-xs leading-5 p-4 text-gray-800 bg-white">
					Parse error or empty content
				</pre>
			)}
		</div>
	);
