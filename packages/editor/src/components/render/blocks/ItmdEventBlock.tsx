...
export const ItmdEventBlock: React.FC<{
  ...
  const destSegments = (() => {
    ...
    if (d.kind === 'fromTo' || d.kind === 'dashPair') {
      const from =
        inlineToSegments(preferAltNames ? d.from_alt || d.from : d.from) || [];
      const to =
        inlineToSegments(preferAltNames ? d.to_alt || d.to : d.to) || [];
      type Segment = { text: string; url?: string; kind?: 'text' | 'code' };
      const viasSrc: any[] = Array.isArray(d.vias) ? d.vias : [];
      const vias = viasSrc
        .map((v) => inlineToSegments(v) || [])
        .filter(
          (segArr: Segment[]): segArr is Segment[] =>
            Array.isArray(segArr) && segArr.length > 0
        );
      return vias.length > 0
        ? { kind: d.kind, from, to, vias }
        : { kind: d.kind, from, to };
    }
    if (d.kind === 'single') {
      const at =
        inlineToSegments(preferAltNames ? d.at_alt || d.at : d.at) || [];
    ...
    for (const p of itmdPrice) {
      ...
      const ws = (
        ...
        (w) =>
          w === 'math-eval-failed' ||
          w === 'math-eval-error' ||
          w === 'currency-not-detected' ||
          w === 'amount-not-detected' ||
          w === 'no-amount' ||
          w === 'unrecognized'
      );
