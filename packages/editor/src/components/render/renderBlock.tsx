...
type ItmdEventNode = {
	...
	time?:
		...
		| { kind: "point"; startISO?: string | null }
		| { kind: "range"; startISO?: string | null; endISO?: string | null };
};
...
export const createRenderBlock = (ctx: RenderBlockContext) => {
	...
	const CodeBlock: React.FC<{
		...
		return (
			<pre
				className="bg-gray-50 border border-gray-200 overflow-x-auto p-2 my-4 ml-20 text-sm"
				{...dataProps}
			>
				<code
					ref={(el) => {
	...
	const renderBlock = (node: MdNode, idx: number): React.ReactNode => {
				...
				return (
					...
					>
						<Heading
							date={date}
							timezone={tz}
							prevStaySegments={prevStaySegments}
						/>
					</div>
				);
			}
		...
		if (type === "itmdAlert") {
			...
			const subtitleEl = inlineTitle ? (
				<>{renderInline(inlineTitle)}</>
			) : undefined;
			...
			const contentEls =
				children?.map((c: any, ci: number) => {
					...
					return (
						<React.Fragment key={key}>{renderBlock(c, ci)}</React.Fragment>
					);
				}) || [];

			return (
				<div
					key={`alert-${lineStart ?? idx}`}
					className="contents"
					{...commonDataProps}
				>
		...
		if (type === "itmdEvent") {
			...
			const baseType =
				(ev as { baseType?: "transportation" | "stay" | "activity" })
					.baseType ?? "activity";
			...
			const arrSegs =
				ev.destination &&
				((ev.destination as any).to || (ev.destination as any).at)
					? inlineToSegments(
				...
				for (const s of body) {
					...
					if (s.kind === "inline") {
						...
						const items = Array.isArray(s.items)
							...
							: [];
						const listSegs = items.map(
							(inline) => inlineToSegments(inline) || [],
						);
			...
			const eventData = {
				...
				departure: depSegs
					? segmentsToPlainText(depSegs) || undefined
					: undefined,
				arrival: arrSegs
		...
		if (type === "paragraph") {
			...
			const pClass = mergeClassNames(
				"mb-4 leading-relaxed text-gray-800 text-base ml-24",
				hClass as string | undefined,
			);
			return (
				<p
					key={`p-${lineStart ?? idx}`}
		...
		if (type === "list") {
			...
			const items = Array.isArray((node as any).children)
			...
			const children = items.map((li, i) => {
				...
				const checkbox = isTask ? (
					<input
						...
						checked={isChecked}
						readOnly
						className="absolute -left-6 top-1 w-4 h-4 rounded border-2 border-gray-400 bg-white accent-blue-600"
					/>
				) : null;
				...
				return (
					<li
						key={`li-${liStart ?? i}`}
						className={liClass}
						data-itin-line-start={liStart ? String(liStart) : undefined}
						data-itin-line-end={liEnd ? String(liEnd) : undefined}
					>
						{checkbox}
			...
			return ordered ? (
				<ol
					...
					className="mb-6 space-y-2 ml-24 list-decimal"
					{...listProps}
				>
					{children}
				</ol>
