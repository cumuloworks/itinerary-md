...
export function registerEventTypeCompletion(monaco: any): {
  dispose: () => void;
} {
  return monaco.languages.registerCompletionItemProvider('mdx', {
    ...
    provideCompletionItems: (model: any, position: any) => {
      ...
      const isAfterNumericTimeSpace =
        /^\s*\[(\d{1,2}:\d{2})\](?:\s*-\s*\[(\d{1,2}:\d{2})\])?\s+[A-Za-z]*$/.test(
          until
        );
      const isAfterAmPmSpace =
        /^\s*\[(AM|PM)\](?:\s*-\s*\[(AM|PM)\])?\s+[A-Za-z]*$/i.test(until);
      if (!(isAfterNumericTimeSpace || isAfterAmPmSpace))
        return { suggestions: [] };
