...
export function coerceTimezoneWithToast(
  tz: unknown,
  fallback: string,
  source: string
): string {
  ...
  notifyError(
    tInstant('toast.tz.coerce', { source, tz: String(tz), fallback })
  );
  return fallback;
}
...
export function getDayOffset(
  ...
  return Math.round(
    target.startOf('day').diff(baseDateTime.startOf('day'), 'days').days
  );
}
