...
export const parseAmountWithCurrency = (
...
): { amount: number | null; currency?: CurrencyCode; raw: string } => {
  ...
  const CODE_AT_END =
    /^([+-]?[0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]+)?|[+-]?[0-9]+(?:\.[0-9]+)?)\s*([A-Za-z]{3})\b/;
  ...
  const CODE_AT_START =
    /^([A-Za-z]{3})\s*([+-]?[0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]+)?|[+-]?[0-9]+(?:\.[0-9]+)?)\b/;
  ...
  const SYMBOL_AND_NUM =
    /^([€¥£₩₫฿₱₽₺$]|A\$|C\$|HK\$|S\$)?\s*([+-]?[0-9]+(?:,[0-9]{3})*(?:\.[0-9]+)?)/;
  ...
  if (Number.isFinite(num))
    return { amount: num, currency: fallbackCurrency, raw: value };
  return { amount: null, currency: fallbackCurrency, raw: value };
};

export const convertAmountUSDBase = (
  amount: number,
  from: CurrencyCode,
  to: CurrencyCode,
  rates: Record<string, number>
...
export const getCurrencyMinorUnit = (
  code: CurrencyCode,
  fallback: number = 2
): number => {
  try {
    ...
    return typeof ro.maximumFractionDigits === 'number'
      ? ro.maximumFractionDigits
      : fallback;
  } catch {
    return fallback;
  }
...
export const formatMoneyCodeSpaced = (
  amount: number,
  code: CurrencyCode,
...
export const formatOriginalWithConverted = (
  amount: number,
  from: CurrencyCode,
  to?: CurrencyCode,
  rates?: Record<string, number>,
  ...
  const fromDigits =
    typeof fractionDigits === 'number'
      ? fractionDigits
      : getCurrencyMinorUnit(fromCode, 2);
  const toDigits =
    typeof fractionDigits === 'number'
      ? fractionDigits
      : getCurrencyMinorUnit(toCode, 2);
...
export const formatConvertedPreferred = (
  amount: number,
  from: CurrencyCode,
  to?: CurrencyCode,
  rates?: Record<string, number>,
  ...
  if (!fromCode || !toCode || fromCode === toCode)
    return formatMoneyCodeSpaced(amount, fromCode, fromDigits);
