...
describe("url utilities", () => {
	...
	describe("isExternalHttpUrl", () => {
		...
		it("not detect when http:// appears in the middle", () => {
			expect(isExternalHttpUrl("text with http://example.com inside")).toBe(
				false,
			);
	...
	describe("isAllowedImageSrc", () => {
		it("allow URLs starting with https", () => {
			...
			expect(isAllowedImageSrc("https://cdn.example.com/assets/img.png")).toBe(
				true,
			);
		...
		it("allow data URLs", () => {
			expect(isAllowedImageSrc("data:image/png;base64,iVBORw0KG...")).toBe(
				true,
			);
			expect(isAllowedImageSrc("data:image/jpeg;base64,/9j/4AA...")).toBe(true);
			expect(isAllowedImageSrc("data:image/svg+xml;base64,PHN2Zy...")).toBe(
				true,
			);
		...
		it("handle special data URL formats", () => {
			expect(isAllowedImageSrc("data:text/plain;charset=utf-8,Hello")).toBe(
				true,
			);
	...
	describe("buildGoogleMapsSearchUrl", () => {
		...
		it("encode non-ASCII place names", () => {
			...
			expect(url).toBe(
				"https://www.google.com/maps/search/?api=1&query=M%C3%BCnchen",
			);
		});
		...
		it("encode spaces correctly", () => {
			...
			expect(url).toBe(
				"https://www.google.com/maps/search/?api=1&query=New%20York%20City",
			);
		});
		...
		it("handle all characters requiring URL encoding", () => {
			...
			expect(url.length).toBeGreaterThan(
				"https://www.google.com/maps/search/?api=1&query=".length,
			);

			// Verify decoding returns the original string
			const encodedPart = url.replace(
