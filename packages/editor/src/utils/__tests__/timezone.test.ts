  ...
  describe('coerceTimezoneWithToast', () => {
    it('returns valid timezone as-is', () => {
      ...
      globalThis.Intl.DateTimeFormat =
        mockDateTimeFormat as unknown as Intl.DateTimeFormatConstructor;
    ...
    it('returns fallback for invalid timezone', () => {
      ...
      const result = coerceTimezoneWithToast(
        'Invalid/Zone',
        'America/New_York',
        'Test'
      );
      ...
      expect(notifyError).toHaveBeenCalledWith(
        'Test: Invalid timezone "Invalid/Zone". Fallback to America/New_York'
      );
    });
    ...
    it('returns fallback when DateTimeFormat throws', () => {
      ...
      expect(notifyError).toHaveBeenCalledWith(
        'Source: Invalid timezone "Bad/Zone". Fallback to Europe/London'
      );
    });
    ...
    it('returns fallback for non-string values', () => {
      ...
      expect(notifyError).toHaveBeenCalledWith(
        'Number input: Invalid timezone "123". Fallback to UTC'
      );
    });
    ...
    it('returns fallback for null', () => {
      ...
      expect(notifyError).toHaveBeenCalledWith(
        'Null input: Invalid timezone "null". Fallback to Asia/Seoul'
      );
    });
    ...
    it('returns fallback for undefined', () => {
      ...
      expect(notifyError).toHaveBeenCalledWith(
        'Undefined input: Invalid timezone "undefined". Fallback to Pacific/Auckland'
      );
    });
    ...
    it('returns fallback for empty string', () => {
      ...
      expect(result).toBe('UTC');
      expect(notifyError).toHaveBeenCalledWith(
        'Empty string: Invalid timezone "". Fallback to UTC'
      );
    });
    ...
    it('returns fallback when resolvedOptions timezone mismatches input', () => {
      ...
      expect(result).toBe('UTC');
      expect(notifyError).toHaveBeenCalledWith(
        'Mismatch: Invalid timezone "Europe/London". Fallback to UTC'
      );
    });
    ...
    it('includes source info in error message', () => {
      ...
      expect(notifyError).toHaveBeenCalledWith(
        expect.stringContaining('Component X:')
      );
    });
    ...
    it('includes fallback value in error message', () => {
      ...
      expect(notifyError).toHaveBeenCalledWith(
        expect.stringContaining('Fallback to Asia/Shanghai')
      );
    });
    ...
    it('stringifies objects for error message', () => {
      ...
      expect(result).toBe('UTC');
      expect(notifyError).toHaveBeenCalledWith(
        'Object test: Invalid timezone "[Object timezone]". Fallback to UTC'
      );
    });
  });
});
