...
export function flattenSegmentsText(
...
export function buildOutlookCalendarUrl(params: {
  ...
  if (params.start)
    q.push(
      `startdt=${encodeURIComponent(params.start.toISO({ suppressSeconds: true, includeOffset: true }) || '')}`
    );
  if (params.end)
    q.push(
      `enddt=${encodeURIComponent(params.end.toISO({ suppressSeconds: true, includeOffset: true }) || '')}`
    );
  if (params.location)
    q.push(`location=${encodeURIComponent(params.location)}`);
