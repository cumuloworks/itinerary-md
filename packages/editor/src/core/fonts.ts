...
export type EditorFontsOption =
  | GoogleFontsConfig
  | StylesheetFontsConfig
  | false
  | undefined;

function buildGoogleFontsHref(config: GoogleFontsConfig): string {
  const familiesParam = config.families
...
export function injectFontsLinks(fonts: EditorFontsOption): () => void {
  ...
  if ((fonts as GoogleFontsConfig).provider === 'google') {
    ...
    const preconnectGstatic = makeLink(
      'preconnect',
    ...
    const preconnectGoogleapis = makeLink(
      'preconnect',
