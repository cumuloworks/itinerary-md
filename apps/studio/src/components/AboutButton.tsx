...
const AboutButton: React.FC = () => {
    ...
    const onBackdropKeyDown = useCallback((e: React.KeyboardEvent<HTMLButtonElement>) => {
        ...
        <div className="flex items-center gap-2 p-2">
            ...
            {open && (
                <div aria-modal="true" role="dialog" className="fixed inset-0 z-50 flex items-center justify-center">
                    <button type="button" aria-label="Close dialog" className="absolute inset-0 bg-black/40" onClick={close} onKeyDown={onBackdropKeyDown} />
                    <div role="document" className="relative bg-white rounded-lg max-w-lg w-[90vw] p-6 shadow-xl">
                        ...
                        <div className="mt-4 flex justify-end gap-2">
                            <button type="button" className="px-3 py-1.5 text-sm rounded border border-gray-300 text-gray-700 hover:bg-gray-50" onClick={close}>
                                Close
                            </button>
                        </div>
                    </div>
