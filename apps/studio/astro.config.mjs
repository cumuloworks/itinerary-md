  ...
  integrations: [
    ...
    pwa({
      ...
      manifest: {
        ...
        description:
          'TripMD Studio is a Markdown editor for composing and sharing travel itineraries.',
        start_url: '/',
        scope: '/',
        display: 'standalone',
        orientation: 'any',
      ...
      workbox: {
        ...
        globPatterns: [
          '**/*.{js,css,html,png,svg,ico,webmanifest,woff,woff2,ttf,otf,md,json}',
        ],
    ...
    plugins: [
      // Force full reload when local workspace packages are rebuilt
      {
        ...
        configureServer(server) {
          const editorDist = fileURLToPath(
            new URL('../../packages/editor/dist', import.meta.url)
          );
          ...
          server.watcher.on('change', (changedPath) => {
            if (
              changedPath.startsWith(editorDist) ||
              changedPath.startsWith(coreDist)
            ) {
